...
def fetch_crypto_data(symbol, start_date, end_date):
    ...
    try:
        df = yf.download(f"{symbol}-USD", start=start_date, end=end_date)
        if df.empty:
...
class SentimentAnalyzer:
    def __init__(self):
        self.api_key = st.secrets.get("NEWS_API_KEY", "")
        if not self.api_key:
            st.warning("News API key not configured. Sentiment analysis will be disabled.")
    ...
    def calculate_portfolio_value(self, portfolio):
        ...
        for crypto, amount in portfolio.items():
            try:
                ...
                errors.append(f"Error fetching {crypto} price: {str(e)}")
                continue
